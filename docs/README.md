...
| **Admin panel** (open round, draw, splits, sponsor tithe, pause) | **No password** (demo only) |
...
> ⚠️ Demo admin has **no password gate** — not real security. Replace with proper auth (wallet + `OPERATOR_ROLE`, or SSO) before any real deployment.
...
flowchart TB
  ...
  subgraph ORA["🔮 Oracle family (oracles.modelmarket.dev)"]
    SO["Sortes — ECVRF draw<br/>sortes.draw@v1 · sortes.verify@v1"]
    PL["Platon — AI Treasurer<br/>platon.ask@v1"]
  ...
  R --- TR
  R -->|"sortes.draw@v1 (draw entropy · ungrindable ECVRF)"| SO
...
|---|---|---|---|
| Sortes | `sortes.draw@v1` | draw entropy — ungrindable ECVRF (RFC 9381), committed at close | $0.006 |
