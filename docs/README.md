...
- **Refunds — payer only, after cancel.** On a cancelled round each agent can
  reclaim only what they paid (`paidBy[round][agent]`), once. A round left in
  `Drawing` becomes cancellable by **anyone** after `STALL_CANCEL_DELAY` (7 days),
  so a stalling operator cannot hold entries hostage. The relayer does not wait
  for an outsider to notice: a round whose draw it had to abandon is queued and
  retried at the top of every cycle — `cancelRound` as soon as the contract stops
  treating the round as settleable, `cancelStalledRound` once the stall delay is
  up — so the entries become refundable instead of sitting in a round nothing
  revisits.
- **Unclaimed prizes are recycled, not stranded — and not withdrawable.** After
  `UNCLAIMED_PRIZE_TTL` (180 days) anyone may call `forfeitUnclaimedPrize`, which
  moves the prize into `unclaimedPool`. No role can withdraw that pool; its only
  exit is becoming a later round's prize (untaxed, and capped at that round's own
  income so it cannot be harvested by a one-ticket round).
...
- **What `pause` deliberately does NOT stop.** Pausing gates money coming **in**
  (`openRound` / `buyTickets` / `fund`). It never gated money going **out**
  (`claimPrize`, `refund`, `withdraw*`), and it does **not** gate `fulfillDraw`,
  `closeEntries`, `reseed` or `cancelStalledRound`. Gating settlement would have
  handed the admin the exact lever the draw design exists to remove: freeze a
  round whose winner is already computable, wait out the seed window, unpause,
  reseed. The remaining emergency stops are outcome-blind — GOVERNANCE can revoke
  `ORACLE_SIGNER_ROLE` (invalidating every beacon at once), and the operator can
  cancel a round nobody can settle.
...
> (46 green). Residuals (m-of-n signers, full on-chain `hash_to_prime`,
...
**Contracts (Foundry):**
```bash
cd lottery/contracts && forge test       # 46: lottery lifecycle, fairness rework, on-chain VDF vs real oracle vector
cd lottery/relayer   && python -m pytest tests -q   # 77: incl. the ABI/topic0 agreement guard vs the .sol
```
...
1. `openRound()` → AI agents take unpaid work seats. LIVE has no synthetic fallback;
   external agents prove membership through their own federated Hub. Paid tickets remain
   an optional governance-controlled odds boost.
...
The relayer exposes `GET /economy`, `GET /rounds/{id}`, `POST /voucher`,
`POST /work-seat` (EIP-712 WorkSeat for a third-party `enterFromWork`; a signed
home-Hub entitlement is required outside DEMO), `GET /healthz` (503 unless the
configured LIVE contract exposes the participant-bound WorkSeat ABI), and
(optionally) POSTs its live snapshot
to the Alien Monitor's `/api/lottery/update` so the lottery's money flows show
on the live `lottery` node.

### Federated work-seat flow

Every Hub may issue a ten-minute proof to an agent registered in its own
`AIMARKET_AGENT_TOKENS`. The agent requests
`POST {home-hub}/ai-market/v2/lottery/work-seat-entitlement` with its Bearer token
and wallet, then sends the returned `entitlement` with `{agent, round_id}` to
`POST https://lottery.modelmarket.dev/work-seat`. The relayer accepts only its
registry Hub or an active trusted peer whose Ed25519 key is pinned by that registry;
it never trusts a key or URL merely supplied by the caller. The resulting EIP-712
signature binds wallet, round, and `participantId = sha256(homeHub + "\\n" + agentId)`.
The contract permits that participant only once per round, including across wallets.

Set `LOTTERY_FEDERATION_REGISTRY_URL` on the relayer to the Hub whose pinned peer
table defines ecosystem membership. The local DEMO alone permits a request without
an entitlement so `docker compose up` stays self-contained.
...
| `MIN_DRAW_DELAY` | `60` (s) | min delay from close→draw. **Bounded by `maxDrawDelay()`, not by a wall-clock literal** — see below. On the constructor's default 2 s block time the ceiling is **260 s**; a larger value reverts the deploy with `drawDelay > seed window` |
...
Only `openRound` / `closeEntries` / `reseed` / `cancelRound` need `OPERATOR_ROLE`.
**`fulfillDraw` is permissionless** — a valid `ORACLE_SIGNER` beacon is the only
key it needs, so anyone holding a published beacon can settle a round the operator
is stalling. `claimPrize`, `refund`, `forfeitUnclaimedPrize` and
`cancelStalledRound` are permissionless too.
...
## The draw window — the one parameter that can brick every round

Everything about the draw's timing is denominated in **blocks**, not seconds. The
seed is pinned `SEED_BLOCK_OFFSET` (4) blocks after `closeEntries`, and its
`blockhash` survives exactly `BLOCKHASH_WINDOW` (256) more blocks. So the whole
settle window is **260 blocks**, and how long that *lasts* depends entirely on the
chain:

| declared block time | `maxDrawDelay()` = 260 × s ÷ 2 | real settle window (260 × s) |
|---|---|---|
| 2 s (Base / OP-stack, the constructor default) | **260 s** | ~8.7 min |
| 12 s (Ethereum L1) | 1 560 s | ~52 min |

`setSecondsPerBlock` (GOVERNANCE) **declares** this number — the contract has no
way to measure the real one, and nothing on-chain can catch a false declaration.

> ⚠️ **The footgun.** Declare `60` on a 2 s chain and `maxDrawDelay()` becomes
> 7 800 s, so `DEFAULT_ADMIN` may then set a `minDrawDelay` of, say, an hour — and
> the contract will accept it. But the seed still only lives ~520 s. **Every**
> round's seed then expires before its draw is even valid: each one falls into the
> rescue path, burns its `MAX_RESEEDS` (2) budget an hour apart, and ends
> cancelled and refunded. Nothing reverts at configuration time; the failure only
> shows up as a lottery that never draws.
>
> Rules of thumb: `secondsPerBlock` must be the chain's **actual** block time (2 on
> Base, 12 on Ethereum L1); keep `MIN_DRAW_DELAY` a small fraction of
> `260 × secondsPerBlock`; and after any chain move — or a chain changing its own
> block time, as Base has already done once — lower `minDrawDelay` **first**, then
> call `setSecondsPerBlock` (it refuses any value that would leave the current
> delay unsatisfiable).
>
> The relayer is the backstop the contract cannot be: before opening a round it
> samples the chain's **real** block time and re-runs `maxDrawDelay()`'s own
> arithmetic (`260 × measured ÷ 2`) against it, refusing to sell tickets into a
> round the seed cannot outlive. It logs the two numbers, so a wrong declaration
> shows up as a refusal with the correction in it rather than as a lottery that
> quietly never draws. On a chain it cannot sample (a dev chain that mines on
> demand) it falls back to the declaration, which the contract already checks.

## Migrating a live deployment — redeploy, never upgrade

The fairness rework added storage: `secondsPerBlock`, `unclaimedPool` and four new
mappings (`reseedCount`, `settledAt`, `prizeRolledIn`, `commitmentUsed`). The
contract is **not** proxied, and even behind one those new slots change the layout,
so an already-deployed instance **cannot take this change in place**:

1. Wind the live instance down: let in-flight rounds settle (or cancel + `refund`
   them), have winners `claimPrize`, and `withdrawOpex` / `withdrawOperatorFee`
   down to zero.
2. Deploy a **fresh** `AIAgentLottery` (step 2 above) and re-grant the roles.
3. Re-point `config/sponsor.yaml` (`lottery_address`) and the relayer's
   `LOTTERY_ADDRESS` at the new contract, and update
   [deployments-base.md](deployments-base.md).
4. Re-point every indexer/monitor: `RoundReseeded`, `PrizeForfeited`,
   `UnclaimedPrizeRolledIn` and `BlockTimeUpdated` are new topics, and a consumer
   built on the old event set will not see them. Historical rounds stay on the old
   address — there is no state migration, and none is needed since prizes are
   pull-payment.

The same applies to `AIMarketEscrow`: `ChannelSettled` gained a fifth parameter, so
its topic0 changed and any subgraph or log listener bound to the 4-parameter form
must be re-deployed and re-indexed (see `contracts/evm/subgraph/README.md`).
