...
contract AIAgentLotteryTest is Test {
    ...
    function setUp() public {
        ...
        vm.prank(governance);
        lot.setPaidTicketsEnabled(true);
    }
        ...
        returns (bytes memory)
    {
        bytes32 structHash = keccak256(
            abi.encode(
                ...
                agent, participantId, roundId, weightBps, expiry
            )
        );
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(signerPk, _typed(structHash));
        return abi.encodePacked(r, s, v);
    ...
    function test_paid_tickets_off_by_default_and_work_seat() public {
        AIAgentLottery fresh = new AIAgentLottery(
            admin, governance, operator, signer, treasury,
            address(0), PRICE, 8000, 1200, 800, 1 hours, 0, false, 0
        );
        assertFalse(fresh.paidTicketsEnabled(), "paid tickets default off");
        vm.deal(agentA, 1 ether);
        vm.prank(operator);
        uint256 id = fresh.openRound();
        vm.prank(agentA);
        vm.expectRevert(AIAgentLottery.PaidTicketsOff.selector);
        fresh.buyTickets{value: PRICE}(id, 1);

        uint64 expiry = uint64(block.timestamp + 1 days);
        bytes32 participantId = keccak256("home-hub:agent-a");
        bytes memory sig = _workSigOn(fresh, agentA, participantId, id, 10_000, expiry);
        vm.prank(agentA);
        fresh.enterFromWork(id, participantId, 10_000, expiry, sig);
        assertTrue(fresh.workSeated(id, agentA));
        assertTrue(fresh.participantSeated(id, participantId));
        assertEq(fresh.participantsCount(id), 1);
        assertEq(fresh.getRound(id).ticketRevenue, 0, "work seat does not buy a ticket");
        assertEq(fresh.getRound(id).totalWeight, 1);
    }

    function _workSigOn(
        AIAgentLottery target,
        address agent,
        bytes32 participantId,
        uint256 roundId,
        uint16 weightBps,
    ...
    function test_double_work_seat_reverts() public {
        uint256 id = _open();
        vm.prank(governance);
        lot.setPaidTicketsEnabled(false);
        uint64 expiry = uint64(block.timestamp + 1 days);
        bytes32 participantId = keccak256("home-hub:agent-a");
        bytes memory sig = _workSig(agentA, participantId, id, 10_000, expiry);
        vm.prank(agentA);
        lot.enterFromWork(id, participantId, 10_000, expiry, sig);
        vm.prank(agentA);
        vm.expectRevert(AIAgentLottery.AlreadyWorkSeated.selector);
        lot.enterFromWork(id, participantId, 10_000, expiry, sig);
    }

    function test_same_hub_participant_cannot_reenter_with_new_wallet() public {
        uint256 id = _open();
        vm.prank(governance);
        lot.setPaidTicketsEnabled(false);
        uint64 expiry = uint64(block.timestamp + 1 days);
        ...
        vm.prank(agentA);
        lot.enterFromWork(
            id, participantId, 10_000, expiry,
            _workSig(agentA, participantId, id, 10_000, expiry)
        );
        vm.prank(agentB);
        vm.expectRevert(AIAgentLottery.AlreadyParticipantSeated.selector);
        lot.enterFromWork(
            id, participantId, 10_000, expiry,
            _workSig(agentB, participantId, id, 10_000, expiry)
        );
    }

    function test_work_seat_then_paid_ticket_stacks() public {
        uint256 id = _open();
        vm.prank(governance);
        lot.setPaidTicketsEnabled(false);
        uint64 expiry = uint64(block.timestamp + 1 days);
        ...
        _buy(agentA, id, 1);
        assertEq(lot.participantsCount(id), 2);
        assertEq(lot.getRound(id).totalWeight, 2);
        assertEq(lot.getRound(id).ticketRevenue, PRICE);
    }

    function test_only_governance_sets_paid_tickets() public {
        vm.prank(operator);
        vm.expectRevert();
        lot.setPaidTicketsEnabled(false);
        vm.prank(governance);
        lot.setPaidTicketsEnabled(false);
        assertFalse(lot.paidTicketsEnabled());
    }
}
