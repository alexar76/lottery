 ...
 *
 * Randomness / fairness:
 *   - Entries close at a recorded block; the seed binds `blockhash` of that block
 *     (unknown when the round opened) ⊕ the Platon oracle randomness, so neither
 *     the operator nor a player can predict or grind the draw.
...
contract AIAgentLottery is AccessControlDefaultAdminRules, Pausable, ReentrancyGuard, EIP712 {
    ...
    function closeEntries(uint256 roundId, bytes32 seedCommitment) external onlyRole(OPERATOR_ROLE) {
        ...
        r.seedBlock = block.number; // future blockhash binds the seed
        r.seedCommitment = seedCommitment;
        emit EntriesClosed(roundId, r.seedBlock);
    }

    /// @notice Re-anchor the seed block while Drawing — rescues a round whose
    ///         seedBlock blockhash expired past the 256-block window (otherwise it
    ///         would be permanently undrawable / funds stuck).
    function reseed(uint256 roundId) external onlyRole(OPERATOR_ROLE) {
        Round storage r = _rounds[roundId];
        if (r.status != Status.Drawing) revert WrongStatus();
        r.seedBlock = block.number;
        emit EntriesClosed(roundId, r.seedBlock);
    }
