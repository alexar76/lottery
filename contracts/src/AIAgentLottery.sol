 ...
 * @notice Protocol tithe pool: Hub routing fees are redistributed to agents by
 *         an unbiasable oracle beacon (Platon chaos-VRF + Chronos VDF). The
 *         default door is a *work seat* (signed Hub-work voucher, no ticket
 *         payment). Optional paid tickets — OFF by default — add weight. LUMEN
 *         reputation may boost odds. Same bytecode for demo / live / uni.
 *
 * Randomness / fairness:
 ...
 * `token == address(0)`.
 *
 * SECURITY/LEGAL: a paid ticket plus chance plus a prize is lottery-shaped under
 * many regulators even if the default door is unpaid work. `paidTicketsEnabled`
 * is OFF at deploy. Turning it on is the operator's legal call — see docs. The
 * in-repo audit is not a substitute for a professional one.
 */
contract AIAgentLottery is AccessControlDefaultAdminRules, Pausable, ReentrancyGuard, EIP712 {
    ...
    bytes32 private constant WORK_SEAT_TYPEHASH =
        keccak256("WorkSeat(address agent,bytes32 participantId,uint256 roundId,uint16 weightBps,uint64 expiry)");
    ...
    // Extra paid tickets that boost odds. OFF at deploy: the default door is
    // enterFromWork. Governance may flip this on; that is a regulatory decision.
    bool public paidTicketsEnabled;
    ...
    // One unpaid work seat per agent per round (tickets may still stack on top).
    mapping(uint256 => mapping(address => bool)) public workSeated;
    // Stable Hub-scoped identity nullifier. A new wallet cannot turn one ecosystem
    // participant into multiple seats in the same round.
    mapping(uint256 => mapping(bytes32 => bool)) public participantSeated;
    ...
    event PaidTicketsSet(bool enabled);
    ...
    error PaidTicketsOff();
    error AlreadyWorkSeated();
    error AlreadyParticipantSeated();
    ...
    /// @notice Governance door for optional paid tickets (odds boost). Default OFF.
    function setPaidTicketsEnabled(bool on) external onlyRole(GOVERNANCE_ROLE) {
        paidTicketsEnabled = on;
        emit PaidTicketsSet(on);
    }

    /// @notice Unpaid work seat: one per Hub-authenticated participant per round.
    ///         A wallet also cannot enter twice. Weight is 1 unit × weightBps/BPS
    ///         (LUMEN may raise weightBps up to BPS+MAX_REP_BONUS_BPS). No ETH/token moves.
    function enterFromWork(
        uint256 roundId,
        ...
        whenNotPaused
    {
        if (weightBps < BPS || weightBps > BPS + MAX_REP_BONUS_BPS) revert WorkWeight();
        if (participantId == bytes32(0)) revert InvalidParticipant();
        if (block.timestamp > expiry) revert VoucherExpired();
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(WORK_SEAT_TYPEHASH, msg.sender, participantId, roundId, weightBps, expiry)
            )
        );
        if (!hasRole(ORACLE_SIGNER_ROLE, ECDSA.recover(digest, sig))) revert BadSignature();
        if (workSeated[roundId][msg.sender]) revert AlreadyWorkSeated();
        if (participantSeated[roundId][participantId]) revert AlreadyParticipantSeated();
        workSeated[roundId][msg.sender] = true;
        participantSeated[roundId][participantId] = true;
        uint256 weight = _seat(roundId, 1, weightBps, 0);
        emit WorkEntered(roundId, msg.sender, participantId, weight);
    }

    /// @notice Buy `count` tickets for the current open round (flat weight).
    ///         Reverts while `paidTicketsEnabled` is false (the default).
    function buyTickets(uint256 roundId, uint256 count) external payable nonReentrant whenNotPaused {
        if (!paidTicketsEnabled) revert PaidTicketsOff();
    ...
    function _buy(uint256 roundId, uint256 count, uint256 weightBps) internal {
        ...
        uint256 weight = _seat(roundId, count, weightBps, received);
        emit TicketsBought(roundId, msg.sender, count, weight, received);
    }

    /// @dev Seat `count` weight-units. `received==0` is a work seat (no refundable tickets).
    function _seat(uint256 roundId, uint256 count, uint256 weightBps, uint256 received)
        internal
    ...
    function fund(uint256 roundId, uint256 amount) external payable nonReentrant whenNotPaused {
        Round storage r = _rounds[roundId];
        // OPEN only. Accepting `Drawing` let capital arrive after `closeEntries` had locked
        // every other participant out, and `fulfillDraw` caps the unclaimed-prize roll-in at
        // this round's own `income = ticketRevenue + funding` — so a sole remaining ticket
        // holder could raise that cap at the moment it already knew it could not be
        // out-competed, which is the precise thing the cap is there to prevent. A late tithe
        ...
        returns (uint256)
    {
        ...
        if (onchainVdf) {
            ...
            // The CANONICAL residue, never the raw calldata. keccak256 of the raw bytes
            // gave the proof's producer a free choice of random word: `verifyEquation` only
            // ever compared `y` as a residue, so `0x00||y` (and, before the companion fix
            // there, `y + k*N`) satisfied the identical proof while hashing differently.
            // The VDF exists to make this draw unbiasable BY THE BEACON SIGNER; deriving
            // the word from bytes that signer chooses is the one way to undo that.
            return uint256(keccak256(ChronosVDF.canonicalY(vdf.y, vdf.N)));
        }
