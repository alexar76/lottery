...
library ChronosVDF {
    ...
    function verifyEquation(
        ...
        // ...and require y to be ALREADY REDUCED. Comparing only the residue meant
        // `y + k*N` satisfied this equation for every k, so one proof had infinitely many
        // accepted witnesses. That is harmless for the equation and not harmless at all for
        // the caller: AIAgentLottery derived its random word from these bytes, so whoever
        // produced the proof could enumerate k after the seed block was mined and pick the
        // word that won. `eq` ignores leading zeros, so an honest `y` — with or without
        // padding — still passes.
        if (!BigMath.eq(yr, y)) return false;
        return BigMath.eq(left, yr);
    }

    /**
     * @notice The canonical residue of a verified `y`, for callers that HASH it.
     * @dev    `verifyEquation` above rejects a `y >= N`, which leaves only the leading-zero
     *         variants — same value, different bytes, different keccak256. Anything deriving
     *         randomness from `y` must hash this, not the raw calldata, or the caller has
     *         handed the prover a free choice among equally valid encodings.
     */
    function canonicalY(bytes memory y, bytes memory N) internal view returns (bytes memory) {
        return BigMath.mod(y, N);
    }
