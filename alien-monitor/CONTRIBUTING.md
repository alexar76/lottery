...
Thanks for helping improve AI-Factory v2.1. This document is the entry point for contributors; deeper operator and security detail lives under [`docs/`](https://github.com/alexar76/alien-monitor/blob/main/docs/README.md).
...
- **Config:** Layered YAML in `config/fragments/` with an optional overlay in [`config.yaml`](https://github.com/alexar76/aicom/blob/main/config.yaml) (intentionally empty in git — Admin → Settings persists overrides there).
...
> **Read [`docs/testing-rules.md`](https://github.com/alexar76/aicom/blob/main/docs/testing-rules.md) first** — codebase-specific testing rules
...
**Pipeline worker locally:** `USE_SQLITE=true .venv/bin/python pipeline_worker.py` (needs configured providers and `data/state/`). The worker uses **event-driven wake** (`signal_new_work()`) with adaptive idle polling — see `AIFACTORY_PIPELINE_*_POLL_SEC` in [`docs/configuration.md`](https://github.com/alexar76/alien-monitor/blob/main/docs/configuration.md).
...
- **Env vars:** document new keys in [`.env.example`](https://github.com/alexar76/alien-monitor/blob/main/.env.example) and [`docs/configuration.md`](https://github.com/alexar76/alien-monitor/blob/main/docs/configuration.md).
- **LLM provider IDs:** canonical names live in [`llm/provider_ids.py`](https://github.com/alexar76/aicom/blob/main/llm/provider_ids.py). Legacy ids (`deep-seek` → `deepseek_api`) are auto-migrated on API startup when `AIFACTORY_AUTO_MIGRATE_PROVIDER_IDS=1` (default). Manual run: `python scripts/migrate_llm_calls_provider_ids.py`.
- **Deploy helper:** [`scripts/fill_production_env.py`](https://github.com/alexar76/aicom/blob/main/scripts/fill_production_env.py) appends **missing** production keys only (never overwrites your `.env`).

## API documentation

- Swagger UI: `/api/docs` (proxied via frontend at `http://localhost:9080/api/docs` in Compose).
- Hand-crafted OpenAPI metadata: [`web/backend/openapi_meta.py`](https://github.com/alexar76/aicom/blob/main/web/backend/openapi_meta.py). Prefer adding `summary` / `description` on new FastAPI routes.
- Integration guide: [`docs/api-integration-guide.md`](https://github.com/alexar76/aicom/blob/main/docs/api-integration-guide.md).
...
- Clarify docs ([`docs/README.md`](https://github.com/alexar76/alien-monitor/blob/main/docs/README.md) index).
...
Use the **good first issue** template when opening starter tasks. Audit follow-ups (no grants): [`docs/audit-remediation.md`](https://github.com/alexar76/aicom/blob/main/docs/audit-remediation.md).
...
| Configuration | [`docs/configuration.md`](https://github.com/alexar76/alien-monitor/blob/main/docs/configuration.md) |
| Pipeline ops | [`docs/pipeline-operations.md`](https://github.com/alexar76/aicom/blob/main/docs/pipeline-operations.md) |
| Admin UI | [`docs/admin-guide.md`](https://github.com/alexar76/aicom/blob/main/docs/admin-guide.md) |
| Agents | [`docs/agents.md`](https://github.com/alexar76/aicom/blob/main/docs/agents.md) |
| Architecture | [`docs/architecture-orchestrator.md`](https://github.com/alexar76/aicom/blob/main/docs/architecture-orchestrator.md) |

Questions about scope? Open an issue before a large refactor. For production deployments, see [`docs/production-domain.md`](https://github.com/alexar76/aicom/blob/main/docs/production-domain.md) and [`README.md`](README.md).
