...
_REP_BONUS_SECRET: str | None = None


def _rep_bonus_secret() -> str:
    """Key for the deterministic stand-in reputation bonus.

    ``LOTTERY_REP_BONUS_SECRET`` makes the stand-in stable across restarts; without it a
    per-process random key is generated, which is still grind-proof (the point) but makes
    an agent's stand-in bonus change when the relayer restarts.
    """
    global _REP_BONUS_SECRET
    if _REP_BONUS_SECRET is None:
        configured = (os.environ.get("LOTTERY_REP_BONUS_SECRET") or "").strip()
        if configured:
            _REP_BONUS_SECRET = configured
        else:
            _REP_BONUS_SECRET = secrets.token_hex(16)
            log.warning(
                "LOTTERY_REP_BONUS_SECRET is unset — using a per-process key, so the "
                "stand-in reputation bonus changes on restart. Set it for stability."
            )
    return _REP_BONUS_SECRET

PRICE = {
    "platon.random@v1": 0.004,
    "chronos.eval@v1": 0.01,
    "lumen.reputation@v1": 0.005,
    "platon.ask@v1": 0.003,
    "sortes.draw@v1": 0.006,
}
PRODUCT = {
    "platon.random@v1": "prod-platon",
    "chronos.eval@v1": "prod-chronos",
    "lumen.reputation@v1": "prod-lumen",
    "platon.ask@v1": "prod-platon",
    "sortes.draw@v1": "prod-sortes",
}


class OracleUnavailable(Exception):
    """Live oracle invoke failed — caller must not substitute grindable randomness."""
...
class OracleClient:
    ...
    def lumen_reputation(
        self, agents: list[str], *, allow_pseudo_fallback: bool = True
    ) -> tuple[dict[str, int], OracleResult]:
        """Returns agent→reputation-bonus-bps (0..5000, the contract's cap).

        ``allow_pseudo_fallback=False`` refuses the deterministic stand-in and answers 0
        when the oracle produced no differentiated score. The /voucher path passes False:
        a single-agent request ALWAYS has lo == hi, so that path never once used a real
        LUMEN score — it signed the stand-in every time, in live mode as much as in demo.
        No differentiated reputation means no boost, not a boost from a hash.
        """
        ...
        if self.cfg.use_live_oracles:
            ...
            bonuses = self._scores_to_bonuses(
                agents, scores, allow_pseudo_fallback=allow_pseudo_fallback
            )
        else:
            r = self._demo(cap, {"nodes": n})
            bonuses = (
                {a: self._pseudo_bonus(a) for a in agents}
                if allow_pseudo_fallback
                else dict.fromkeys(agents, 0)
            )
        self.ledger.record(r)
        return bonuses, r

    def sortes_draw(self, client_seed: str) -> tuple[bytes, OracleResult]:
        """ECVRF draw — raises OracleUnavailable in live mode when the oracle is down."""
        cap = "sortes.draw@v1"
        if self.cfg.use_live_oracles:
            try:
                r = self._invoke_strict(cap, {"alpha": client_seed, "num_bytes": 32})
    ...
    def _invoke_strict(self, capability_id: str, payload: dict) -> OracleResult:
        """Live invoke without demo fallback — for security-critical draws (Sortes)."""
        price = PRICE.get(capability_id, 0.0)
        if self.cfg.hub_url:
            body = {
                "product_id": PRODUCT.get(capability_id, ""),
            ...
            resp = self._http.post(
                f"{self.cfg.hub_url}/ai-market/v2/invoke", json=body, headers=headers
            )
            resp.raise_for_status()
            data = resp.json()
            price = float(data.get("price_usd", price) or price)
        ...
        if self.cfg.oracle_url:
            ...
            value = self._unwrap_output(data)
            return OracleResult(value=value, price_usd=price, routing_fee_usd=0.0, source="live-direct")
        raise OracleUnavailable("no hub_url or oracle_url configured for live draw")
    ...
    @staticmethod
    def _scores_to_bonuses(
        agents: list[str], scores, *, allow_pseudo_fallback: bool = True
    ) -> dict[str, int]:
        def _fallback() -> dict[str, int]:
            if not allow_pseudo_fallback:
    ...
    def _pseudo_bonus(agent: str) -> int:
        """Deterministic per-agent stand-in bonus, KEYED so it cannot be ground offline.

        This used to be ``keccak("rep|" + agent) % 5001`` — a pure function of a public
        address. Since /voucher signs whatever bonus comes back, an attacker generated
        keypairs locally until one hashed to 5000 (uniform over 0..5000, so ~5k tries,
        milliseconds) and then asked the relayer to sign a maximum +50% odds voucher for
        an address it controlled. Keying it with a server-side secret makes the search
        require a request per candidate, which the /voucher rate limit then bounds.
        """
        h = int(Web3.keccak(text=f"rep|{_rep_bonus_secret()}|{agent}").hex(), 16)
