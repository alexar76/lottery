...
Two paths, both faithful to AIAgentLottery._randomWord / _verifyBeacon:
  ...
  onchain_vdf = False (default, tested):
    the proof is EMPTY; the contract's random word IS the baseSeed. `prevrandao`
    is no longer mixed in — it added no entropy against the signer (commit-reveal
    already pins platonRandom, and the blockhash is unknown at commit time) while
    making the winner a function of the SUBMISSION block, i.e. a re-roll lever.
    The beacon still commits the (empty) proof hash so the signature binds the
    whole payload.
