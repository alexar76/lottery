...
#: Module-level alias so the HTTP layer can catch the same set without importing the class.
_ABI_MISMATCH = (
...
from .rpc import connect_web3, parse_rpc_urls
...
class Chain:
    def __init__(self, rpc_url: str, chain_id: int, lottery_address: str,
        ...
        urls = list(rpc_urls) if rpc_urls else parse_rpc_urls(rpc_url)
        self.rpc_url = urls[0]
        self.rpc_urls = urls
        self.w3 = connect_web3(urls)
    ...
    def paid_tickets_enabled(self) -> bool:
        try:
            return bool(self.contract.functions.paidTicketsEnabled().call())
        except Exception:
            return True

    def work_seated(self, rid: int, agent: str) -> bool:
        try:
            return bool(self.contract.functions.workSeated(
                rid, Web3.to_checksum_address(agent)).call())
        except Exception:
            return False

    def participant_seated(self, rid: int, participant_id: str) -> bool:
        # Do not collapse "old contract has no participantSeated selector" into
        # "this participant is not seated". The caller must be able to fail closed
        # instead of signing a WorkSeat the configured contract cannot consume.
        return bool(self.contract.functions.participantSeated(
            rid, Web3.to_bytes(hexstr=participant_id)).call())

    #: An ABI/decode failure means the DEPLOYED contract lacks the selector. Anything
    #: else (timeout, 429, node down) is a transport problem and must not be reported as
    #: "redeploy your contract" — that flipped /healthz to 503 and 503'd every work seat
    #: whenever the RPC hiccuped, which the compose healthcheck turned into a restart loop.
    _ABI_MISMATCH = _ABI_MISMATCH

    def supports_federated_work_seats(self) -> bool:
        """Probe the new participant-bound ABI with a side-effect-free mapping read.

        Returns False only for a genuine ABI mismatch; a transport error propagates so
        the caller can say "RPC unavailable" instead of blaming the deployment.
        """

        try:
            self.contract.functions.participantSeated(0, b"\x00" * 32).call()
        except self._ABI_MISMATCH:
            return False
        return True

    def entry_window(self) -> int:
        return int(self.contract.functions.entryWindow().call())
    ...
    def sign_work_seat(self, private_key: str, agent: str, participant_id: str, round_id: int,
                       weight_bps: int, expiry: int) -> bytes:
        typed = {
            "types": {
                "EIP712Domain": [
                    {"name": "name", "type": "string"},
                ...
                "WorkSeat": WORK_SEAT_TYPE,
            },
            "primaryType": "WorkSeat",
            "domain": self._domain(),
            "message": {
                "agent": Web3.to_checksum_address(agent),
                "participantId": Web3.to_bytes(hexstr=participant_id),
                "roundId": round_id,
                "weightBps": int(weight_bps),
                "expiry": expiry,
            },
        }
        signable = encode_typed_data(full_message=typed)
