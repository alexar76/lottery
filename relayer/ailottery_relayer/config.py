...
def _is_address(v: str) -> bool:
    v = (v or "").strip()
    if not (len(v) == 42 and v.startswith("0x")):
        return False
    try:
        int(v, 16)
    except ValueError:
...
# Keys an env-snippet may use for the lottery, most specific first. The UNI bootstrap writes
# all three; a wrapper that grepped for exactly one of them silently fell back to a hardcoded
# address when the snippet happened to spell it differently.
_LOTTERY_KEYS = ("LOTTERY_ADDRESS", "evm_lottery", "lottery_address", "address")
_HUB_LOTTERY_KEYS = ("HUB_LOTTERY_ADDRESS", "AIMARKET_CHARITY_LOTTERY_ADDRESS",
                     "hub_lottery_address")


def read_address_source(path: str) -> dict[str, str]:
    """Read the deploy step's address file in whichever shape it was written.

    Three shapes are in the wild and all three are legitimate:
      * a bare address (`/shared/lottery.address`, written by the demo deploy step),
      * an env snippet of `KEY=value` lines (the UNI bootstrap's `hub.env.snippet`),
      * a JSON config (`universe_config.json`, keyed `evm_lottery`).
    Anything unreadable is "no answer", never an exception: the relayer must keep driving the
    address it already has rather than die because a shared mount blinked.
    """
    out: dict[str, str] = {}
    try:
    ...
    if _is_address(text.splitlines()[0].strip()):
        return {"lottery": text.splitlines()[0].strip()}
    ...
    if text.startswith("{"):
        ...
        for line in text.splitlines():
            ...
            doc[key.strip()] = val.split("#", 1)[0].strip().strip('"').strip("'")

    for keys, slot in ((_LOTTERY_KEYS, "lottery"), (_HUB_LOTTERY_KEYS, "hub")):
        for key in keys:
...
class Config:
    ...
    rpc_urls: list[str] = field(default_factory=lambda: ["http://chain:8545"])
    chain_id: int = 31337
    lottery_address: str = ""                # set, or read from address_file
    address_file: str = "/shared/lottery.address"
    # How often the engine re-checks that its lottery address still holds code. The bubble
    # re-bootstraps its Anvil from scratch now and then, and the address it hands out changes.
    # 0 = every round; negative = never (LOTTERY_ADDRESS_RECHECK_S).
    address_recheck_s: float = 30.0
    ...
    sim_agents: bool = True                  # relayer drives work seats (or tickets if enabled)
    sim_tickets_per_round: int = 4
    wait_for_fund: bool = False              # live: do not close/draw an unfunded round
    entry_window_s: int = 0                 # 0 = leave on-chain; else setEntryWindow at boot
    work_seat_count: int = 4                 # named work seats when Mesh is empty (live)
    ...
    def from_env(cls) -> "Config":
        ...
        from .rpc import parse_rpc_urls, with_live_fallbacks
        c.rpc_urls = with_live_fallbacks(
            parse_rpc_urls(os.getenv("RPC_URL", c.rpc_url)), c.mode == "live")
        c.rpc_url = c.rpc_urls[0]
        ...
        c.address_recheck_s = _f("LOTTERY_ADDRESS_RECHECK_S", c.address_recheck_s)
        ...
        c.uni_auto_wallet = _b("UNI_AUTO_WALLET", c.mode in ("uni", "live"))
        # deterministic per-agent keys: seed defaults to the operator key (UNI-local secret)
        c.uni_wallet_seed = (
            os.getenv("UNI_WALLET_SEED", "") or os.getenv("WALLET_SEED", "")
            or c.operator_key or "ailottery-uni"
        )
        c.faucet_key = os.getenv("FAUCET_KEY", "") or c.sponsor_key or c.operator_key
        live_gas = 3 * 10**14  # ~0.0003 ETH — enough for a few Base work-seat txs, not a stake
        c.agent_fund_wei = _i("AGENT_FUND_WEI", live_gas if c.mode == "live" else 5 * 10**16)
        c.wait_for_fund = _b("WAIT_FOR_FUND", c.mode == "live")
        c.entry_window_s = _i("ENTRY_WINDOW_S", 86_400 if c.mode == "live" else 0)
        c.work_seat_count = _i("WORK_SEAT_COUNT", 0 if c.mode == "live" else c.work_seat_count)
        ...
        if c.mode == "live" and not os.getenv("MESH_MAX_AGENTS"):
            c.mesh_max_agents = min(c.mesh_max_agents, 4)
        ...
        c.round_interval = _f("ROUND_INTERVAL", 20.0 if c.mode == "live" else c.round_interval)
        c.sell_window = _f("SELL_WINDOW", 0.0 if c.mode == "live" else c.sell_window)
    ...
    def resolved_address(self) -> str:
        ...
        return read_address_source(self.address_file).get("lottery", "")

    def file_addresses(self) -> dict[str, str]:
        """What the address FILE says right now — re-read every time, never cached.

        The file is the deployer's live answer; the env is a snapshot of that answer taken
        when the container was created. When the two disagree the file is the newer one.
        """
        return read_address_source(self.address_file)

    def address_candidates(self) -> list[str]:
        """Every address this deployment could legitimately drive, best first.

        Order matters: the operator's explicit env first (it is what they asked for), then
        whatever the deploy step has written to the address file since. Callers pick the
        first candidate that actually holds contract code — see
        `EconomyEngine._ensure_contract_live`.
        """
        found = self.file_addresses()
        out: list[str] = []
        for cand in (self.lottery_address, found.get("lottery", ""),
                     found.get("hub", ""), self.hub_lottery_address):
            addr = (cand or "").strip()
