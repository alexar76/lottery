...
from .eligibility import participant_id
from .config import Config
from .abi import STATUS_CANCELLED, STATUS_DRAWING, STATUS_OPEN, STATUS_SETTLED
from .economy_draw import (
    defer_stalled,
...
def derive_agent_key(seed: str, agent_id: str) -> str:
    ...
    h = Web3.keccak(text=f"ailottery-uni-wallet|{seed}|{agent_id}").hex()
...
_LIVE_SEAT_NAMES = ["AuditHawk", "NetPulse", "CodeNova", "DeepScout", "LedgerOwl"]
WORK_WEIGHT_BPS = 10_000
MAX_WORK_BONUS_BPS = 5_000


def live_work_roster(seed: str, n: int = 5) -> list[Participant]:
    """Named work-seat wallets when Mesh is empty — still real keys, gas-funded, on-chain."""
    if n <= 0:
        return []
    out = []
    for i, name in enumerate(_LIVE_SEAT_NAMES[:n]):
        key = derive_agent_key(seed, f"work-seat|{name}|{i}")
        addr = Account.from_key(key).address
        out.append(Participant(name=name, key=key, addr=addr, wallet=addr, source="work"))
    return out
...
class EconomyEngine:
    def __init__(self, cfg: Config):
        ...
        self.chain = Chain(cfg.rpc_url, cfg.chain_id, addr,
                           rpc_urls=getattr(cfg, "rpc_urls", None))
        ...
        self._running = True
        self._last_address_check = 0.0   # 0 → verify on the first round, not one interval in
        self.contract_live = True        # what /healthz reports (see _ensure_contract_live)
        # Rounds a draw gave up on while the contract still refused a cancel. They
        # hold real entries, and nothing else in the relayer revisits a round once
        # the cycle moves on, so the sweep at the top of run_one_round retries the
        # contract's exits until each one is cancelled (or settled by somebody else).
        self.stalled_rounds: list[int] = []
        ...
        # an explicit local work roster in LIVE; synthetic agents only outside LIVE.
        # Rebuilt each round so newly-registered agents appear live.
        ...
        self._align_entry_window()
        self._seed_state()

    def _seed_state(self) -> None:
        """Roster-only feed so /economy is honest even if the first RPC snapshot 429s."""
        self.state = {
            "mode": self.cfg.mode,
            ...
            "prize_pool_usd": 0.0,
            "metrics": {
                "prize_pool_usd": 0, "round": 0, "players": 0,
                "payouts_24h": 0, "opex_24h": 0, "funding_24h": 0,
            },
            "oracle_ledger": {"spend_usd": 0, "routing_fees_usd": 0, "calls": []},
            "reserve_usd": 0.0,
            "treasurer": None,
            "sponsor": {
                "bound": getattr(self.binding, "ok", False),
                "address": getattr(self.binding, "address", self.chain.address),
                "reason": getattr(self.binding, "reason", ""),
                "tithe_bps": self.sponsor.tithe_bps() if hasattr(self, "sponsor") else 0,
            ...
            "paid_tickets": False,
            "wait_for_fund": bool(self.cfg.wait_for_fund),
            "roster": [
    ...
    def _align_entry_window(self) -> None:
        want = int(self.cfg.entry_window_s or 0)
        if want <= 0:
            return
        try:
            have = self.chain.entry_window()
            if have == want:
                return
            self.chain.send(self.fn.setEntryWindow(want), self.operator_key)
            log.info("entryWindow %ss → %ss (so a live round can wait for fund())", have, want)
        except Exception as exc:
            log.warning("setEntryWindow failed: %s", exc)

    # ── roster (real Mesh agents, else synthetic) ───────────────────────────────
    def _refresh_roster(self) -> None:
        """Rebuild the participant roster from the Mesh; keep the last good Mesh roster
        on a transient outage; use an explicitly configured showcase roster otherwise.
        ...
        LIVE defaults to zero fallback seats: public participation goes through a Hub
        entitlement and the HTTP work-seat endpoint instead."""
        ...
        if self.cfg.mode == "live":
            self._roster = live_work_roster(self.cfg.uni_wallet_seed, self.cfg.work_seat_count)
            self._fund_and_bind_roster()
            self._name_by_addr = {p.addr: p.name for p in self._roster}
            return
    ...
    def roster_source(self) -> str:
        if any(p.source == "mesh" for p in self._roster):
            return "mesh"
        if any(p.source == "work" for p in self._roster):
            return "work"
        if any(p.source == "synthetic" for p in self._roster):
            return "synthetic"
        return "none"
    ...
    # ── the address the chain can move under us ────────────────────────────────
    def _has_code(self, addr: str) -> bool:
        code = self.chain.w3.eth.get_code(Web3.to_checksum_address(addr))
        return bool(code) and len(code) > 0

    def _rebind(self, addr: str) -> None:
        old = self.chain.address
        self.chain = Chain(self.cfg.rpc_url, self.cfg.chain_id, addr,
                           rpc_urls=getattr(self.cfg, "rpc_urls", None))
        self.fn = self.chain.contract.functions
        self.cfg.lottery_address = self.chain.address
        # The anti-redirect gate compares the sponsor's configured binding against the
        # operating lottery. Adopt the new address as that binding ONLY when the same file
        # that named it also names it as the Hub's — a stale env alone must never be able to
        # point the tithe somewhere new.
        found = self.cfg.file_addresses()
        if (found.get("hub") or found.get("lottery") or "").lower() == addr.lower():
            self.cfg.hub_lottery_address = self.chain.address
        self.binding = self.sponsor.resolve_bound(self.chain.address, self.chain.w3)
        # Rounds abandoned mid-draw belong to the OLD contract; their ids mean something
        # different (or nothing) on the new one, so sweeping them would cancel strangers.
        self.stalled_rounds = []
        log.warning("re-bound lottery %s → %s (sponsor binding ok=%s: %s)",
                    old, self.chain.address, self.binding.ok, self.binding.reason)
        self._event("operator", "rebind", f"lottery {self.chain.address}", 0)

    def _ensure_contract_live(self, force: bool = False) -> bool:
        """Re-point at the lottery when the chain redeployed it under us.

        The relayer resolved its address ONCE, at startup. The UNI bubble re-bootstraps its
        Anvil from scratch now and then and every contract gets a new address, so the relayer
        kept driving an address that no longer held code — and a call to a code-less address
        does not revert: it "succeeds", burns gas and does nothing. That failure is invisible
        from the outside (it once ran for nine days), so the address is re-checked here
        instead of being trusted for the life of the process.
        """
        # 0 = check every round; a NEGATIVE value is the opt-out. ("0 disables it" would be
        # the one setting an operator picks when they mean "check as often as possible".)
        if self.cfg.address_recheck_s < 0 and not force:
            return True
        now = time.time()
        if not force and now - self._last_address_check < self.cfg.address_recheck_s:
            return True
        self._last_address_check = now
        try:
            if self._has_code(self.chain.address):
                self.contract_live = True
                return True
        except Exception as exc:
            # An RPC blip is not a redeploy — keep driving what we have and re-check later.
            log.warning("lottery code check failed (%s) — keeping %s", exc, self.chain.address)
            return True

        log.error("no contract code at %s — the chain redeployed the lottery under us",
                  self.chain.address)
        for cand in self.cfg.address_candidates():
            if cand.lower() == self.chain.address.lower():
                continue
            try:
                if not self._has_code(cand):
                    continue
            except Exception:
                continue
            self._rebind(cand)
            self.contract_live = True
            return True
        log.error("no live lottery among %s — refusing to drive rounds against a dead address",
                  self.cfg.address_candidates() or ["(no candidates)"])
        self.contract_live = False
        return False
    ...
    def sell_tickets(self, rid: int) -> None:
        """Seat the roster. Default door is unpaid work; paid tickets only if the flag is on."""
        if not self.cfg.sim_agents:
            return
        self._refresh_roster()
        if not self._roster:
            return
        if self.chain.paid_tickets_enabled():
            self._buy_tickets(rid)
        else:
            self._enter_from_work(rid)

    def _enter_from_work(self, rid: int) -> None:
        now = self.chain.block()["timestamp"]
        expiry = now + 86_400
        for p in self._roster:
            bonus = min(int(p.trust_bps or 0), MAX_WORK_BONUS_BPS)
            weight = WORK_WEIGHT_BPS + bonus
            try:
                identity = p.mesh_id or p.name or p.addr
                # NOT self.cfg.hub_url. participant_id(hub_url, id) is the nullifier the
                # Hub derives for an agent it AUTHENTICATED; a Mesh id is self-declared,
                # so sharing the namespace let a mesh agent calling itself "agent-7" take
                # the on-chain seat belonging to the Hub's real agent-7. Keep the
                # relayer's own roster in a namespace the Hub never signs into.
                participant = participant_id(
                    f"{(self.cfg.hub_url or 'http://lottery.local').rstrip('/')}/mesh",
                    identity,
                )
                if self.chain.work_seated(rid, p.addr):
                    continue
                if self.chain.participant_seated(rid, participant):
                    continue
                sig = self.chain.sign_work_seat(
                    self.signer_key, p.addr, participant, rid, weight, expiry
                )
                self.chain.send(
                    self.fn.enterFromWork(rid, participant, weight, expiry, sig), p.key
                )
                self._event(p.name, "work-seat", "lottery", 0)
            except Exception as exc:
                log.warning("agent %s work seat failed: %s", p.name, exc)

    def _buy_tickets(self, rid: int) -> None:
    ...
    def apply_sponsor_tithe(self, rid: int) -> None:
        # LIVE: the Hub must PUSH real tithe. Do not invent routing-fee ETH from the operator.
        if self.cfg.mode == "live":
            return
    ...
    def close_and_draw(self, rid: int) -> bool:
        # no players → cancel rather than revert on NoParticipants. `fund()` may
        # still have put sponsor money in the round and only a cancel makes it
        # refundable, so a cancel we could not land is queued for the sweep.
        if self.chain.participants_count(rid) == 0:
            log.info("round %s had no participants → cancel", rid)
            if not safe_cancel(self, rid, "no participants"):
                defer_stalled(self, rid)
            return False

        self._advance_to_close(rid)

        # 1) obtain the Platon entropy and COMMIT it (before the seed block exists).
        #    A rescue needs entropy this round has NEVER committed, so the client
        #    seed carries the rescue attempt.
        def _platon_word(attempt: int = 0) -> bytes:
            client_seed = f"{self.chain.address}|{rid}" if attempt == 0 \
                else f"{self.chain.address}|{rid}|reseed{attempt}"
            w, _ = self.oracles.platon_random(client_seed)
            self._event("lottery", "invoke", "Platon", PRICE["platon.random@v1"])
            return w

        platon_word = _platon_word()
        ...
        # 2) pass minDrawDelay and make sure the pinned seed hash is one the
        #    CONTRACT can still read — rescuing it with fresh entropy if not.
        self.chain.mine()
        settleable = ensure_settleable_seed(self, rid, platon_word, _platon_word)
        if settleable is None:
            if not safe_cancel(self, rid, "no settleable seed"):
                defer_stalled(self, rid)
            return False
        r, platon_word = settleable

        # 3) build the (optional) VDF proof
        if self.chain.onchain_vdf():
            bh = self.chain.seed_blockhash(r["seedBlock"])
            if bh is None:  # raced out of the 256-block window since the check
            ...
            if proof is None:
                ...
                # (the contract requires onchainVdf proofs). Cancel when the contract
                # still allows it; otherwise the round stays settleable/refundable.
                ...
                if not safe_cancel(self, rid, "no valid Chronos proof"):
                    defer_stalled(self, rid)
                return False
            vdf_t = difficulty
        else:
        ...
        # 4) sign the beacon (binds the exact proof) and reveal/draw. Settlement is
        #    permissionless now — the beacon opens it, not the caller's role; we
        #    submit with the operator key only because it is the funded one.
    ...
    def snapshot(self) -> dict:
        ...
        return {
            ...
            "roster_source": self.roster_source,
            "paid_tickets": bool(self.chain.paid_tickets_enabled()),
            "wait_for_fund": bool(self.cfg.wait_for_fund),
            "roster": [
                {"name": p.name, "address": p.addr, "trust_bps": p.trust_bps,
                 "source": p.source, "mesh_id": p.mesh_id, "wallet": p.wallet}
    ...
    def publish(self) -> None:
        try:
            ...
            log.warning("snapshot failed (%s) — keeping last good feed", exc)
            if not self.state:
                self._seed_state()
            return
    ...
    def run_one_round(self) -> None:
        # Before anything touches the contract: is the contract still THERE? (see
        # _ensure_contract_live — the bubble redeploys and every call silently no-ops.)
        if not self._ensure_contract_live():
            return
        # Refund anything an earlier cycle had to abandon mid-draw BEFORE locking
        # fresh entries behind it — those rounds hold real tickets and sponsor
        # funding, and no other component revisits them.
        self.stalled_rounds = sweep_stalled_rounds(self, self.stalled_rounds)
        # Fail closed BEFORE selling tickets: a draw delay the seed cannot outlive
        # would make every round undrawable, and players would be paying into it.
        if not preflight_draw_window(self):
            return
        rid = self.chain.current_round_id()
        rnd = self.chain.get_round(rid) if rid else None
        status = int(rnd["status"]) if rnd else 0
        if status == STATUS_DRAWING:
            log.info("round %s still drawing — not opening another", rid)
            self.publish()
            return
        if rid == 0 or status in (STATUS_SETTLED, STATUS_CANCELLED, 0):
            rid = self.open_round()
            self.publish()
        self.sell_tickets(rid); self.publish()
        self.apply_sponsor_tithe(rid); self.apply_uni_benefactor(rid); self.publish()
        rnd = self.chain.get_round(rid)
        if self.cfg.wait_for_fund and int(rnd.get("funding") or 0) <= 0:
            log.info("round %s seated, waiting for fund() — not closing an empty pot", rid)
            return
    ...
    def run_forever(self) -> None:
        ...
        while self._running:
            try:
                ...
                # A failed round is the loudest symptom of a lottery that moved (every read
                # raises BadFunctionCallOutput against an address with no code), so re-check
                # the address on the NEXT round instead of waiting out the interval.
