...
_MON = Path(__file__).resolve().parents[3] / "alien-monitor" / "backend"
if not (_MON / "live_lottery_feed.py").is_file():
    import pytest

    pytest.skip(
        "alien-monitor backend not present (lottery satellite CI)",
