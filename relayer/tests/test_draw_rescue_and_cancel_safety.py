...
def _production_engine(chain_kwargs, **kw):
    ...
    # The round loop now re-checks that the lottery still holds code before touching it
    # (see test_address_selfheal); these doubles have no chain to ask, so it always says yes.
    engine._ensure_contract_live = lambda: True
    return engine, sent
