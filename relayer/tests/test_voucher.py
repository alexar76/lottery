...
def test_voucher_is_rate_limited_per_agent() -> None:
    ...
    server_mod._voucher_global.clear()
...
def _reset_voucher_buckets() -> None:
    import ailottery_relayer.server as server_mod
...
def test_two_callers_behind_the_proxy_do_not_share_the_caller_bucket(monkeypatch) -> None:
    """The caller bucket used to key on request.client.host, which behind
    lottery/deploy/nginx-relayer-api.conf is the proxy — one 60/hour bucket for the whole
    internet, so any visitor could lock the federation out of vouchers for an hour."""
    import ailottery_relayer.server as server_mod

    _reset_voucher_buckets()
    monkeypatch.setenv("LOTTERY_TRUSTED_PROXIES", "testclient")
    engine = _FakeEngine(bonus=10)
    client = TestClient(make_app(engine))

    # One caller spends its own 60, using a fresh agent every 12 so the agent cap is not
    # what stops it.
    codes = []
    for i in range(server_mod._VOUCHER_MAX_PER_IP + 1):
        agent = "0x" + f"{i:040x}"
        codes.append(
            client.post(
                "/voucher", json={"agent": agent}, headers={"X-Forwarded-For": "203.0.113.9"}
            ).status_code
        )
    assert codes.count(200) == server_mod._VOUCHER_MAX_PER_IP
    assert codes[-1] == 429

    # A second caller is unaffected — before the fix it inherited the exhausted bucket.
    other = client.post(
        "/voucher",
        json={"agent": "0x" + "ab" * 20},
...
def test_priced_oracle_calls_have_a_stated_global_ceiling(monkeypatch) -> None:
    """Agent addresses are free to mint and source addresses are rentable, so neither
    per-key bucket bounds what the operator pays LUMEN. Before the caller bucket was fixed
    it doubled as a global cap by accident; the ceiling is now explicit."""
    import ailottery_relayer.server as server_mod

    _reset_voucher_buckets()
    monkeypatch.setenv("LOTTERY_TRUSTED_PROXIES", "testclient")
    monkeypatch.setattr(server_mod, "_VOUCHER_MAX_GLOBAL", 5)
    engine = _FakeEngine(bonus=10)
    client = TestClient(make_app(engine))

    codes = []
    for i in range(7):
        # A fresh agent AND a fresh caller each time: only the global bucket can stop this.
        codes.append(
            client.post(
                "/voucher",
                json={"agent": "0x" + f"{i:040x}"},
                headers={"X-Forwarded-For": f"203.0.113.{i}"},
            ).status_code
        )
    assert codes.count(200) == 5
    assert codes[-1] == 429
    assert engine.oracles.calls == 5, "a 429 must not buy an oracle call"


def test_forwarded_header_from_an_undeclared_peer_is_ignored(monkeypatch) -> None:
    import ailottery_relayer.server as server_mod

    _reset_voucher_buckets()
    monkeypatch.delenv("LOTTERY_TRUSTED_PROXIES", raising=False)
    engine = _FakeEngine(bonus=10)
    client = TestClient(make_app(engine))

    # No declared proxy: a caller cannot hand itself a fresh bucket by naming an address.
    codes = []
    for i in range(server_mod._VOUCHER_MAX_PER_IP + 1):
        codes.append(
            client.post(
                "/voucher",
                json={"agent": "0x" + f"{i:040x}"},
                headers={"X-Forwarded-For": f"203.0.113.{i % 200}"},
            ).status_code
        )
    assert codes[-1] == 429
